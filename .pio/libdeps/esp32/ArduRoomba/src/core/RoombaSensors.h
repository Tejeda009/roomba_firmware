...
/**
 * Overcurrent motor data
 */
struct OvercurrentData {
  bool left;
  bool right;
  bool mainBrush;
  bool vacuum;
  ...
  bool lowDriver1;
  bool lowDriver2;
  bool anyOvercurrent() const { return left || right || mainBrush || vacuum || sideBrush || leftWheel || rightWheel || lowDriver0 || lowDriver1 || lowDriver2; }
  bool isObstacle() const { return leftWheel || rightWheel; }
};
...
public:
  ...
  // Request a stream of packets form sensors
  bool getStream(uint8_t* sensorIds, uint8_t* data, uint8_t number, uint8_t dataSize);

  bool switchStream(bool state);
