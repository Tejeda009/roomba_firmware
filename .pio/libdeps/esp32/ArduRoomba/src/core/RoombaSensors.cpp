...
uint8_t RoombaSensors::readDirt(){
  uint8_t value;

  if (getSensor(SENSOR_DIRT_DETECT, &value, 1)){
    return value;
  }
...
OvercurrentData RoombaSensors::readCurrent(bool quick = false) {
  ...
  uint8_t value;
  if (!quick) {
    if (getSensor(SENSOR_OVERCURRENT, &value, 1)) {
      data.sideBrush = (value & 0x01) != 0;
      data.vacuum = (value & 0x02) != 0;
      data.mainBrush = (value & 0x03) != 0;
      data.right = (value & 0x04) != 0;
      data.left = (value & 0x05) != 0;
    }
  }

  if (getSensor(SENSOR_OVERCURRENT_OTHER, &value, 1)) {
    data.lowDriver0 = (value & 0x01) != 0;
    data.lowDriver1 = (value & 0x02) != 0;
    data.lowDriver2 = (value & 0x03) != 0;
    data.rightWheel = (value & 0x04) != 0;
    data.leftWheel = (value & 0x05) != 0; 
  }
...
RoombaSensorData RoombaSensors::readAll() {
  ...
  data.overcurrent = readCurrent(false);
  data.lastUpdate = millis();

  return data;
}

bool RoombaSensors::isObstacle() {
  OvercurrentData b = readCurrent(true);
  return b.isObstacle();
}

bool RoombaSensors::isOvercurrent() {
  OvercurrentData b = readCurrent(false);
  return b.anyOvercurrent();
}

bool RoombaSensors::switchStream(bool state){
  if (!_serial || !_serial->isActive()) return false;

  _serial->write(OI_SWITCH_STREAM);
  _serial->write(state);
  _serial->flush();
...
bool RoombaSensors::getStream(uint8_t* sensorIds, uint8_t* data, uint8_t number, uint8_t dataSize){
  if (!_serial || !data || !_serial->isActive()) return false;

  _serial->write(OI_STREAM);
  _serial->write(number);

  for(int i = 0; i<number; i++){
    _serial->write(sensorIds[i]);
  }
  ...
  for(;;){
    readBytes(data, dataSize, 100*number);
  }
}
